...
async def build_pylons(self):
    if self.can_afford(UnitTypeId.PYLON) and ((self.supply_left <= 2) or (self.supply_left <= 2 * (
            len(self.structures(UnitTypeId.NEXUS)) + len(self.structures(UnitTypeId.GATEWAY))))):
        if self.can_afford(UnitTypeId.PYLON) and self.already_pending(UnitTypeId.PYLON) + self.units.filter(
...
async def zealotAndArmy(self):
    for gw in self.structures(UnitTypeId.GATEWAY).ready.idle:
        if self.can_afford(UnitTypeId.STALKER) and len(self.structures(UnitTypeId.CYBERNETICSCORE)) != 0 and len(self.units(UnitTypeId.STALKER)) <=2:
            self.do(gw.train(UnitTypeId.STALKER), subtract_cost=True, subtract_supply=True)
        elif self.can_afford(UnitTypeId.DARKTEMPLAR) and len(self.structures(UnitTypeId.DARKSHRINE)) != 0:
...
async def build_gateway(self):
    if self.can_afford(UnitTypeId.GATEWAY) and self.already_pending(UnitTypeId.GATEWAY) + self.units.filter(
            lambda structure: structure.type_id == UnitTypeId.PYLON and structure.is_ready).amount == 0 and (
            len(self.structures(UnitTypeId.GATEWAY)) < 2 + 3 * (len(self.structures(UnitTypeId.NEXUS)) - 1) and len(
            self.structures(UnitTypeId.GATEWAY)) <= 12):
        map_center = self.game_info.map_center
        position_towards_map_center = self.start_location.towards(map_center, distance=5)
...
async def techUp(self):
    if len(self.structures(UnitTypeId.GATEWAY)) >= 1 and self.can_afford(UnitTypeId.CYBERNETICSCORE) and len(
            self.structures(UnitTypeId.CYBERNETICSCORE)) < 1:
        map_center = self.game_info.map_center
        position_towards_map_center = self.start_location.towards(map_center, distance=5)
        await self.build(UnitTypeId.CYBERNETICSCORE, near=position_towards_map_center, placement_step=1)

    if len(self.structures(UnitTypeId.CYBERNETICSCORE)) == 1 and self.can_afford(UnitTypeId.TWILIGHTCOUNCIL) and len(
            self.structures(UnitTypeId.TWILIGHTCOUNCIL)) < 1:
        map_center = self.game_info.map_center
        position_towards_map_center = self.start_location.towards(map_center, distance=5)
        await self.build(UnitTypeId.TWILIGHTCOUNCIL, near=position_towards_map_center, placement_step=1)

    if len(self.structures(UnitTypeId.TWILIGHTCOUNCIL)) == 1 and self.can_afford(UnitTypeId.DARKSHRINE) and len(
            self.structures(UnitTypeId.DARKSHRINE)) < 1:
        map_center = self.game_info.map_center
        position_towards_map_center = self.start_location.towards(map_center, distance=5)
